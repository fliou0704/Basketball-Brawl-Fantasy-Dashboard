...
COLUMNS = ['Year', 'Week', 'Team Name', 'Type', 'Score', 'Opponent Team Name', 'Opponent Owner']
PLAYER_STATS = ('FPTS', 'PTS', 'REB', 'AST', 'STL', 'BLK', '3PM')
...
def matchup_details(players, year, week, team_ids, team_names):
    ...
    for team_id in team_ids:
        ...
        sides.append([{'name': r['Player Name'], **{stat.lower(): player_value(r[stat]) for stat in PLAYER_STATS}}
...
def perspective(league, players, first, second):
    ...
    history = history[COLUMNS + ['Win', 'Loss']].sort_values(['Year','Week'], ascending=[False,False])
    for row in history.to_dict('records'):
        ...
        result['history'].append({'id': f"historical-{int(row['Year'])}-{int(row['Week'])}",
                                  'fields': {key: row[key] if pd.notna(row[key]) else None for key in COLUMNS},
...
def theoretical(league, players, first, second):
    names = current_names(league)
    eligible = league[league['Type'] != 'Consolation']
    ...
    result = {'team1Id': first, 'team2Id': second, 'title': f'{names[first]} vs. {names[second]}',
              'record': None, 'history': [], 'message': None}
    if rows.empty:
        result['message'] = 'These teams do not have any comparable weeks.'
        return result
    wins = int((rows['Points For A'] > rows['Points For B']).sum())
    losses = int((rows['Points For A'] < rows['Points For B']).sum())
    ties = int(len(rows) - wins - losses)
    result['record'] = {'wins': wins, 'losses': losses, 'ties': ties,
    ...
    for row in rows.to_dict('records'):
        ...
        fields = {'Year': int(row['Year']), 'Week': int(row['Week']), 'Team Name': row['Team Name A'],
                  'Type': row['Type A'], 'Score': f'{int(a)} - {int(b)}',
                  'Opponent Team Name': row['Team Name B'], 'Opponent Owner': row['Team Owner B']}
        result['history'].append({'id': f"theoretical-{int(row['Year'])}-{int(row['Week'])}", 'fields': fields,
                                  'winnerTeamId': winner, 'playoff': row['Type A']=='Playoffs',
                                  'details': matchup_details(players, row['Year'], row['Week'], (first, second),
