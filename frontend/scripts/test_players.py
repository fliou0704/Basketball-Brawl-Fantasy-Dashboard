...
from generate_players import aggregate_career, build


class PlayerCareerExportTests(unittest.TestCase):
    def test_active_played_rows_only_and_multi_team_order_and_totals(self):
        daily = pd.DataFrame([
            {"Year": 2026, "Date": "2025-11-03", "Team ID": 7, "FPTS": 20, "PTS": 10, "REB": 4, "AST": 4, "STL": 4, "BLK": 0, "3PM": 2, "TO": -2, "FGM": 8, "FGA": -16, "FTM": 2, "FTA": -2},
            {"Year": 2026, "Date": "2025-10-20", "Team ID": 2, "FPTS": 30, "PTS": 14, "REB": 5, "AST": 6, "STL": 0, "BLK": 4, "3PM": 1, "TO": -4, "FGM": 10, "FGA": -20, "FTM": 3, "FTA": -4},
        ])
        ...
        self.assertEqual([row["team"]["teamId"] for row in rows],[2,7])
        self.assertEqual(total["starts"],2)
        self.assertEqual(total["FPTS"],sum(row["FPTS"] for row in rows))
    ...
    def test_return_to_a_prior_team_remains_a_separate_stint(self):
        base={"Year":2026,"FPTS":10,"PTS":1,"REB":1,"AST":2,"STL":0,"BLK":0,"3PM":0,"TO":0,"FGM":2,"FGA":-2,"FTM":0,"FTA":0}
        daily=pd.DataFrame([{**base,"Date":"2025-10-20","Team ID":2},{**base,"Date":"2025-10-21","Team ID":7},{**base,"Date":"2025-10-22","Team ID":2}])
        teams={2:{"teamId":2},7:{"teamId":7}}
        rows,_=aggregate_career(daily,teams)
        self.assertEqual([row["team"]["teamId"] for row in rows],[2,7,2])
