...
const stats=['starts','FPTS','PTS','REB','AST','STL','BLK','3PM','TO','FGM','FGA','FTM','FTA'];
const labels={starts:'GP',FPTS:'FPTS',PTS:'PTS',REB:'REB',AST:'AST',STL:'STL',BLK:'BLK','3PM':'3PM',TO:'TO',FGM:'FGM',FGA:'FGA',FTM:'FTM',FTA:'FTA'};
...
function CareerTable({rows,career}) {
  ...
  return <TableCard className="career-table-card"><div className="career-scroll"><table className="career-table"><thead><tr><th>Season</th><th className="career-team">Fantasy team</th>{stats.map(stat=><th key={stat} className="numeric">{labels[stat]}</th>)}</tr></thead><tbody>
    {rows.map((row,index)=><tr key={`${row.season}-${row.team.teamId}-${row.firstDate}-${index}`}><td>{row.season}</td><th scope="row"><span className="career-team-cell"><img src={`${base}${row.team.logo}`} alt=""/><span>{row.team.teamName}</span></span></th>{stats.map(stat=><td key={stat} className={`numeric ${stat==='FPTS'?'career-fpts':''}`}>{row[stat].toLocaleString()}</td>)}</tr>)}
    <tr className="career-total"><th scope="row">Career</th><td>All teams</td>{stats.map(stat=><td key={stat} className={`numeric ${stat==='FPTS'?'career-fpts':''}`}>{career[stat].toLocaleString()}</td>)}</tr>
